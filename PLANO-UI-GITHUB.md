...
3. **Revisado em 2026-09-23 (pedido da Nilma):** a Consulta rápida (chat com IA) abre pela caixa "Perguntar à IA"
   da barra de cima (atalho `/`) e se abre como uma barra de pesquisa aberta: a pergunta em cima, a conversa
   embaixo; no PC é uma caixa ancorada na barra, no celular a tela toda. Sai o botão redondo do canto.
...
- **Clientes (Pendências) no formato das Issues do GitHub.** No alto ficam o título e o mês. Abaixo, o botão "Filtros ▾" colado na busca, com o formato da lista, ⋯ e "Cobrar visíveis" à direita. A lista virou uma caixa: o cabeçalho traz as situações com contador em pílula (Todos, Com pendência, Completos, Sem e-mail) e a "Ordem". O cabeçalho de colunas saiu, e o painel de filtros agora só mostra o que falta e as opções extras.
- **Configurações (Pendências) como as Settings do GitHub.** No PC (≥1000px) as seções ficam num menu à esquerda, com a barra vermelha na seção ativa. Cada bloco ganhou título sublinhado em vez de cartão, e a prévia virou uma caixa com cabeçalho. A prévia fica ao lado só em tela larga (≥1440px); fora isso vai para baixo. No celular as seções continuam como abas.
- **Visão anual no estilo do gráfico de contribuições.** A lista está numa caixa com o filtro no cabeçalho, e cada mês é um quadrado colorido (verde forte = completo, verde claro = faltando, vermelho = nada, cinza = sem registro, tracejado = mês que ainda não chegou). Uma legenda fica no rodapé, e o título de cada quadrado diz o mês e a situação.
- **Cobranças e Robô (Pendências).**
